...
fixed_point_precision_16_16 sqrt_val(fixed_point_precision_16_16 num){
   ...
   while( (i*i) <= num )
          i++;
    i--; 
   fixed_point_precision_16_16 d = num - i*i; 
 fixed_point_precision_16_16 p=d/(2*i); 
 fixed_point_precision_16_16 a=i+p; 
   return a-(p*p)/(2*a);
...
/*
	...
	Problem_Box sdp(Matrix C, Matrix Xp, Matrix_List A, Vector b, fixed_point_precision_16_16 theta, fixed_point_precision_16_16 beta){
	  ...
	  if (p && (d_equal(dot(A.m0, Xp),b.v[0])) && (d_equal(dot(A.m1, Xp),b.v[1]))){
	    ...
	    fixed_point_precision_16_16 stop = (fixed_point_precision_16_16)1.0;
	    fixed_point_precision_16_16 err = (fixed_point_precision_16_16)0.0;

            int loop = 0;
	    while((loop < 1) && !(err > (fixed_point_precision_16_16)1e-2) && !(stop <= (fixed_point_precision_16_16)0.25)){
                loop++;
        ...
        for (int yd=0; yd<n; yd++){
          int max_j = yd;
          for (int rd = yd; rd < n; rd++){
            if (abs_val(get_big(rd,yd,LL)) > abs_val(get_big(max_j,yd,LL,(N*N + M)))){
              max_j = rd;
            }
          }
	    ...
	    fixed_point_precision_16_16 errs = 0.0;
            int loop2 = 0;
	    while((loop2 < 1) && !(errs > 1e-2) && !(theta < 1e-4)){
              loop++;
