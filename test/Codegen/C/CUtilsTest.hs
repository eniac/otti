...
cutilsTest = benchTestGroup
  ...
  [ benchTestCase "new var" $ do
    a <- Assert.execAssert $ Mem.execMem $ do
      Mem.initMem
      cppDeclVar True AST.U8 "my_u8"
    (2 + 1) @=? M.size (Assert.vars a)
  , benchTestCase "new vars" $ do
    a <- Assert.execAssert $ Mem.execMem $ do
      Mem.initMem
      _ <- cppDeclVar True AST.U8 "my_u8"
      cppDeclVar True AST.S8 "my_i8"
    (2 + 2 + 1) @=? M.size (Assert.vars a)
  , benchTestCase "cppAdd: u8 + i8 = i8" $ do
    a <- Assert.evalAssert $ Mem.evalMem $ do
      Mem.initMem
      u <- cppDeclVar True AST.U8 "my_u8"
      i <- cppDeclVar True AST.S8 "my_i8"
      return $ cppAdd u i
    AST.S8 @=? cppType a
  , benchTestCase "cppAdd: i32 + i8 = i32" $ do
    a <- Assert.evalAssert $ Mem.evalMem $ do
      Mem.initMem
      u <- cppDeclVar True AST.S32 "my_i32"
      i <- cppDeclVar True AST.S8 "my_i8"
      return $ cppAdd u i
    AST.S32 @=? cppType a
  ...
  , benchTestCase "cppCond: bool ? u8 : i8 = u8" $ do
    a <- Assert.evalAssert $ Mem.evalMem $ do
      ...
      u <- cppDeclVar True AST.U8 "my_u8"
      i <- cppDeclVar True AST.S8 "my_i8"
      b <- cppDeclVar True AST.Bool "my_bool"
  ...
  --    Mem.initMem
  --    u <- cppDeclVar True AST.U8 "my_u8"
  --    p <- cppDeclVar True (AST.Ptr32 AST.U8) "my_u8_ptr"
  ...
  --  a <- Assert.evalAssert $ Mem.evalMem $ do
  --    Mem.initMem
  --    u <- cppDeclVar True (AST.Ptr32 AST.U8) "my_u8_ptr"
  --    p <- cppDeclVar True (AST.Ptr32 (AST.Ptr32 AST.U8)) "my_u8_ptr_ptr"
  --    _ <- Mem.liftAssert $ cppAssign True p (cppIntLit AST.U32 0)
  --    _ <- cppStore p u (Ty.BoolLit True)
  --    cppLoad p
