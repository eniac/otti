...
genCtxWithSignals sigNames =
  ctxWithEnv (Map.fromList (map (\s -> (s, signalTerm s [])) sigNames)) prime

genCtxWithScalars :: KnownNat k => [(String, Int)] -> Ctx k
genCtxWithScalars pairs = ctxWithEnv
  (Map.fromList (map (\(s, i) -> (s, constTerm $ fromIntegral i)) pairs))
  prime
...
circomGenTests = benchTestGroup
  "Circom generator tests"
  [ genExprTest (genCtxWithSignals []) (NumLit 5) (constBundle 5)
  , genExprTest (genCtxWithSignals [])
                (BinExpr Shl (NumLit 5) (NumLit 2))
                (constBundle 20)
  , genExprTest (genCtxWithSignals [])
                (BinExpr Div (NumLit 5) (NumLit 1))
                (constBundle 5)
  , genExprTest (genCtxWithSignals [])
                (BinExpr Div (NumLit 5) (NumLit 2))
                (constBundle 114)
  , genExprTest (genCtxWithSignals [])
                (BinExpr IntDiv (NumLit 5) (NumLit 2))
                (constBundle 2)
  , genExprTest
    (genCtxWithSignals ["in"])
    (BinExpr Add
             (LValue $ LocalLocation ("in", []))
             (LValue $ LocalLocation ("in", []))
    )
  ...
  , genExprTest
    (genCtxWithSignals ["in"])
    (BinExpr Add
             (BinExpr Mul (LValue $ LocalLocation ("in", [])) (NumLit 5))
             (LValue $ LocalLocation ("in", []))
    )
  ...
  , genExprTest (genCtxWithSignals [])
                ...
                (constBundle 3)
  , genExprTest (genCtxWithSignals [])
                (UnExpr UnNeg (NumLit 5))
                (constBundle (-5))
  , ctxStoreGetTest "integer"
                    (ctxFromList (Map.fromList [("in", Array [constTerm 5])]))
                    (LTermIdent "in")
                    (constTerm 6)
                    (LTermIdent "in")
                    (constTerm 6)
  , ctxStoreGetTest "array to integer"
                    (ctxFromList (Map.fromList [("in", Array [constTerm 5])]))
                    (LTermIdent "in")
                    (constTerm 6)
                    (LTermIdent "in")
                    (constTerm 6)
  , ctxStoreGetTest "array"
                    (ctxFromList (Map.fromList [("in", Array [constTerm 5])]))
  ...
  , ctxStoreGetTest
    "struct in array"
    (ctxFromList
      (Map.fromList [("in", Array [Struct $ genCtxWithScalars [("a", 5)]])])
    )
  ...
  , genStatementsTest
    ...
    [ Constrain (LValue (LocalLocation ("a", [])))
                (LValue (LocalLocation ("b", [])))
    ]
    (ctxAddConstraint
      ( lcZero
      , lcZero
      , (Map.fromList [(SigLocal "a" [], 1), (SigLocal "b" [], -1)], 0)
      )
      (genCtxWithSignals ["a", "b"])
    )
  , genStatementsTest
    ...
    [ AssignConstrain
        (LocalLocation ("a", []))
        (BinExpr Mul (NumLit 2) (LValue (LocalLocation ("b", []))))
    ]
    (ctxAddConstraint
      ( lcZero
      , lcZero
  ...
  , genStatementsTest
    ...
    (ctxFromList $ Map.fromList
      [ ("n"  , constTerm 2)
      , ("in" , signalTerm "in" [])
      , ("out", Array [signalTerm "out" [0], signalTerm "out" [1]])
      ]
    )
  , genStatementsTest
  ...
  , genNewLowDegCoundTest "test/Code/Circom/bitify4.circom" 5
  ]
...
ctxStoreGetTest
  :: String -> Ctx 223 -> LTerm -> Term 223 -> LTerm -> Term 223 -> BenchTest
ctxStoreGetTest name ctx sLoc sVal gLoc gVal =
  benchTestCase ("store/get test: " ++ name) $ do
...
genStatementsTest name ctx s expectCtx' =
  benchTestCase ("statements: " ++ name) $ do
...
genMainTest path expectedConstraints =
  benchTestCase ("main gen: " ++ path) $ do
    m <- Parser.loadMain path
    let constraints = equalities (genMain m prime)
    unless (constraints == expectedConstraints)
      $  error
      $  "Expected\n\t"
...
genMainTestCountOnly path exCs exPubSigs exPrivSigs =
  benchTestCase ("signal & constraint counts for circuit at " ++ path) $ do
    m <- Parser.loadMain path
    ...
    let
      eqs :: [ Constraint
        ...
        = Constraints.equalities constraints
    length eqs @?= exCs
    length (Constraints.public constraints) @?= exPubSigs
    length (Constraints.private constraints) @?= exPrivSigs

genMainAndConvert :: String -> Int -> BenchTest
genMainAndConvert path exSignals =
  benchTestCase ("conversion to Smt for " ++ path) $ do
    m <- Parser.loadMain path
    let
    ...
    depth predicate @?= (exSignals + 5)

type Order = 21888242871839275222246405745257275088548364400416034343698204186575808495617

genNewLowDegCoundTest :: FilePath -> Int -> BenchTest
genNewLowDegCoundTest path expectedNumConstraints =
  benchTestCase ("constraints in " ++ path) $ do
    main <- Parser.loadMain path
    let actualNumConstraints :: Int =
          Seq.length $ Link.constraints $ Link.linkMain @Order main
    actualNumConstraints @?= expectedNumConstraints
