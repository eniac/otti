...
type Assertions n = [Constraint n]
type ConstrId = Int
...
shouldVisit (a, b, c) = a == lcZero || b == lcZero
...
run r1cs = do
  ...
  whileJustM popFromQueue $ \id -> do
    ...
    whenJust (asLinearSub (publicInputs r1cs) c) $ \(v, t) -> do
      liftLog $ logIf "r1cs::opt::lin::sub" $ show $ map show (numSigs r1cs IntMap.! v)
...
reduceLinearities r1cs = do
  ...
  state <- execStateT runAction $ initRedLinState r1cs
  let constraints' =
        Seq.fromList $ filter (not . constantlyTrue) $ IntMap.elems $ view
          cs
          state
