...
def run_sdp(dataset_path):
	for f in dataset_path:
		if f.endswith(".dat-s"):
			...
			subprocess.run(["mv", f+".c", "cfiles/"])

			print("compiling R1CS for " ++ f)
			subprocess.run(["C_outputs="+f+"zkif", "stack", "run", "--", "main", "cfiles/"+f, "--emit-r1cs"])
			subprocess.run(["C_outputs="+f+"zkif", "stack", "run", "--", "main", "cfiles/"+f, "--prove", "-i", '<(echo "")'])
			subprocess.run(["mv", f+"inp.zkif", "zkif/"])
			subprocess.run(["mv", f+"wit.zkif", "zkif/"])
			subprocess.run(["mv", f+".zkif", "zkif/"])
